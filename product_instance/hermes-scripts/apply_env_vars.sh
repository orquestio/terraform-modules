#!/bin/bash
# =============================================================================
# apply_env_vars.sh — Bulk-apply user env vars to the Hermes container
#
# Contract:
#   apply_env_vars.sh '<json_array>'
#     json_array: [{"name":"FOO","value":"bar","is_secret":true}, ...]
#   exit 0  → container.env rewritten, container recreated, health passing
#   exit !=0 → failure (bad args/name, missing container, restart failed)
#
# Semantics:
#   - REPLACE (not merge) for the CUSTOMER portion: the resulting
#     container.env contains exactly the vars in the JSON, plus every
#     infra-owned RESERVED line preserved from the existing file
#     (API_SERVER_*, HERMES_DASHBOARD_*, PUID/PGID/TZ — never user-managed).
#   - Last-wins on duplicate names within the JSON array.
#   - Names must match ^[A-Z_][A-Z0-9_]*$. Attempting to set any RESERVED
#     name via this script is rejected with a clear error.
#   - Idempotent: same input → same file contents. restart.sh runs once at
#     the end regardless (a plain `docker restart` would NOT re-read the
#     --env-file, hence the full recreate).
# =============================================================================
set -euo pipefail
...
SCRIPTS_DIR="/opt/hermes/scripts"
...
# Infra-owned vars for the Hermes engine — the health API listener/key, the
# loopback dashboard binding, and container identity/timezone. Never
# user-managed; overwriting any of these could take the wall or the health
# probe down.
RESERVED = {
    "API_SERVER_ENABLED",
    "API_SERVER_HOST",
...
# Preserve every RESERVED line from the existing file, in original order.
preserved_lines = []
if os.path.exists(env_file):
    with open(env_file, "r", encoding="utf-8") as f:
        for raw in f:
            line = raw.rstrip("\n")
            key = line.split("=", 1)[0]
            if "=" in line and key in RESERVED:
